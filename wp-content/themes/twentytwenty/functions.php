...
function twentytwenty_skip_link_focus_fix() {
	...
	?>
	<script>
	/(trident|msie)/i.test(navigator.userAgent)&&document.getElementById&&window.addEventListener&&window.addEventListener("hashchange",function(){var t,e=location.hash.substring(1);/^[A-z0-9_-]+$/.test(e)&&(t=document.getElementById(e))&&(/^(?:a\vert{}select\vert{}input\vert{}button\vert{}textarea)$/i.test(t.tagName)||(t.tabIndex=-1),t.focus())},!1);
	</script>
	<?php
}
...
function twentytwenty_get_custom_logo( $html ) {
	...
	if ( $logo ) {
		...
		if ( get_theme_mod( 'retina_logo', false ) ) {
			...
			if ( false === strpos( $html, ' style=' ) ) {$search[]  = '/(src=)/';
...
function twentytwenty_sidebar_registration() {
    ...
    // Footer #3 (Được thêm cho Module 3)
	register_sidebar(
		array_merge(
			$shared_args,
			array(
...
function twentytwenty_block_editor_settings() {
	...
	if ( ! $background_color ) {$background_color_arr = get_theme_support( 'custom-background' );
